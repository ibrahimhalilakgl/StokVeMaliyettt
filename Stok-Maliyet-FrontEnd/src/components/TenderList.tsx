...
const TenderList: React.FC = () => {
  ...
  const submitEdit = async () => {
    ...
    try {
      await axios.put(`/tender/update/${selectedTenderId}`, {
  ...
  const submitDelete = async () => {
    if (!selectedTenderId) return;
    try {
      await axios.delete(`/tender/delete/${selectedTenderId}`);
      setDeleteOpen(false);
      fetchTenders();
    } catch (e) {
  ...
  const handleIncreaseTender = () => {
    ...
    axios
      .post(`/tender/increaseTender/${selectedTenderId}/${increaseAmount}`)
      .then((res) => {
        if (res.data.success) {
          if (isCancel) {
