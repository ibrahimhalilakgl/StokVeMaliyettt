...
const YemekhaneHomePage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const ticketTypesResponse = await axios.get('/ticketType/all');
      setTicketTypes(ticketTypesResponse.data.data || []);

      // Bugünkü satışlar - gerçek database verisi
      const today = new Date().toISOString().split('T')[0];
      const todaySalesResponse = await axios.get('/ticketSalesDetail/getTicketByDate', {
        params: {
          startDate: today,
          endDate: today
        }
  ...
  const fetchWeeklySalesTrend = async () => {
    try {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        try {
          const response = await axios.get('/ticketSalesDetail/getTicketByDate', {
            params: {
              startDate: dateStr,
              endDate: dateStr
            }
  ...
  const fetchPendingDemands = async () => {
    try {
      const response = await axios.get('/materialDemand/all');
