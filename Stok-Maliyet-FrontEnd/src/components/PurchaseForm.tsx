...
const PurchaseForm: React.FC = () => {
  ...
  const [itemToDelete, setItemToDelete] = useState<PurchaseFormData | null>(null);

  const fetchData = async () => {
    try {
      const res = await axios.get('/purchaseForm/all');
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('/purchaseForm/create', { name });
      setName('');
      setOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`/purchaseForm/update/${selectedId}`, { name });
      setName('');
      setEditOpen(false);
      setSelectedId(null);
      fetchData();
      ...
      if (err.response && err.response.status === 409) {
        setErrorMessage(err.response.data.data || 'Bu isimde bir alım şekli zaten mevcut.');
        setShowError(true);
      } else {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/purchaseForm/delete/${itemToDelete.id}`);
  ...
  return (
    <Box sx={{ p: 3, position: 'relative' }}>
      ...
      <Dialog open={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)}>
        <DialogTitle>Alım Şeklini Sil</DialogTitle>
        <DialogContent>
          <Typography>
            "{itemToDelete?.name}" alım şeklini silmek istediğinizden emin misiniz? 
