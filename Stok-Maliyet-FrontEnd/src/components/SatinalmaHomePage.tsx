...
const SatinalmaHomePage: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const tenderResponse = await axios.get('/tender/all');
      const tenders = tenderResponse.data.data || [];
      setLastTenders([...tenders].slice(-3).reverse());
      
      // Doğrudan temin verilerini getir
      const directProcurementResponse = await axios.get('/directProcurement/all');
      ...
      const budgetResponse = await axios.get('/budget/all');
      ...
      const stockResponse = await axios.get('/materialEntry/getAllProductDetail');
      ...
      try {
        const demandsResp = await axios.get('/materialDemand/all');
      ...
      const spendResp = await axios.get('/materialEntry/spend-by-budget');
