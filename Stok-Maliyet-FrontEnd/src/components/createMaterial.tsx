...
const CreateMaterial: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Kategorileri çek
        const categoriesResponse = await axios.get('/v1/category/all');
        setCategories(categoriesResponse.data.data || []);

        // Ölçü birimlerini çek
        const measurementTypesResponse = await axios.get('/v1/measurementType/all');
        setMeasurementTypes(measurementTypesResponse.data.data || []);

        // Mevcut malzemeleri çek (silme için seçim)
        const materialsResponse = await axios.get('/v1/product/all');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await axios.post('/v1/product/create', productCreateRequest);
      setShowSuccess(true);
      resetForm();
    } catch (error: any) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/v1/product/delete/${selectedMaterialId}`);
