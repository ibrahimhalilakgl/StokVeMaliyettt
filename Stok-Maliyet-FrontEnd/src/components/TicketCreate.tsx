...
const handleQuantityChange = (ticketTypeId: number, value: string) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post('/ticketSalesDetail/create', requestBody);
  ...
  const handleResetByDate = async () => {
    ...
    try {
      await axios.delete(`/ticketSalesDetail/deleteByDate?date=${ticketDate}`);
