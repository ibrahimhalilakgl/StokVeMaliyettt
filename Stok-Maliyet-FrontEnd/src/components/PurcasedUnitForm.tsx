...
const PurchaseUnit: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('/v1/purchasedUnit/all');
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('/v1/purchasedUnit/create', { name });
      setName('');
      setOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`/v1/purchasedUnit/update/${selectedId}`, { name });
      setName('');
      setEditOpen(false);
      setSelectedId(null);
      fetchData();
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/v1/purchasedUnit/delete/${itemToDelete.id}`);
