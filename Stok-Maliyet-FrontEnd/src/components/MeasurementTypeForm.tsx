...
const MeasurementTypeForm: React.FC = () => {
  const [list, setList] = useState<MeasurementType[]>([]);
  ...
  const [itemToDelete, setItemToDelete] = useState<MeasurementType | null>(null);

  const fetchData = async () => {
    try {
      const res = await axios.get('/measurementType/all');
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('/measurementType/create', { name });
      setName('');
      setOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`/measurementType/update/${selectedId}`, { name });
      setName('');
      setEditOpen(false);
      setSelectedId(null);
      fetchData();
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/measurementType/delete/${itemToDelete.id}`);
  ...
  return (
    <Box sx={{ p: 3, position: 'relative' }}>
        ...
        <Table>
          <TableHead sx={{ backgroundColor: '#f5f5f5' }}>
            <TableRow>
              ...
              <TableCell sx={{ fontSize: '1rem', fontWeight: 'bold' }}>Ölçü Birimi</TableCell>
      ...
      {/* Yeni Ölçü Birimi Ekleme Dialog */}
      <Dialog open={open} onClose={() => setOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Yeni Ölçü Birimi Ekle</DialogTitle>
      ...
      <Dialog open={editOpen} onClose={() => setEditOpen(false)} maxWidth="sm" fullWidth>
        <DialogTitle>Ölçü Birimi Güncelle</DialogTitle>
        <DialogContent dividers>
          <TextField
            label="Ölçü Birimi Adı"
            fullWidth
      ...
      <Dialog open={deleteConfirmOpen} onClose={() => setDeleteConfirmOpen(false)}>
        <DialogTitle>Ölçü Birimini Sil</DialogTitle>
        <DialogContent>
          <Typography>
            "{itemToDelete?.name}" ölçü birimini silmek istediğinizden emin misiniz? 
