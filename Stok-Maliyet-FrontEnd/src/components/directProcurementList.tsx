...
const DirectProcurementList: React.FC = () => {
  ...
  const fetchDirectProcurements = () => {
    const url =
      tabIndex === 0
        ? '/v1/directProcurement/activeDirectProcurement'
        : '/v1/directProcurement/all';
  ...
  const submitEdit = async () => {
    ...
    try {
      await axios.put(`/v1/directProcurement/update/${selectedDirectProcuremntId}`, {
  ...
  const submitDelete = async () => {
    if (!selectedDirectProcuremntId) return;
    try {
      await axios.delete(`/v1/directProcurement/delete/${selectedDirectProcuremntId}`);
      setDeleteOpen(false);
      fetchDirectProcurements();
  ...
  const handleIncreaseDirectProcurement = () => {
    ...
    axios
      .post(`/v1/directProcurement/increaseDirectProcurement/${selectedDirectProcuremntId}/${increaseAmount}`)
      .then((res) => {
        if (res.data.success) {
