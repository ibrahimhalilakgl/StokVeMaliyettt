...
const PurchaseType: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const res = await axios.get('/purchaseType/all');
  ...
  const handleSubmit = async () => {
    try {
      await axios.post('/purchaseType/create', { name });
      setName('');
      setOpen(false);
      fetchData();
    } catch (err: any) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      await axios.put(`/purchaseType/update/${selectedId}`, { name });
      setName('');
      setEditOpen(false);
      setSelectedId(null);
      fetchData();
  ...
  const handleDelete = async () => {
    ...
    try {
      await axios.delete(`/purchaseType/delete/${itemToDelete.id}`);
