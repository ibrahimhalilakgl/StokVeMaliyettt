...
const AdminHomePage: React.FC = () => {
  ...
  useEffect(() => {
    axios.get('/v1/materialEntry/all')
      .then((res) => {
...
const handleCategoryClick = (formName: string) => {
  ...
  useEffect(() => {
    axios.get('/v1/materialEntry/spend-by-budget')
      .then(res => {
  ...
  axios
    .get(`/v1/ticketSalesDetail/getTicketByDate?startDate=${formattedYesterday}&endDate=${formattedYesterday}`)
    .then((res) => {
      ...
      {budgetSpending.map((item, index) => {
        ...
        return (
          <Grid item xs={12} sm={colSize} key={index}>
            ...
            >
              <Typography variant="subtitle2" sx={{ fontWeight: 600, fontSize:'20px', color: color.text, mb: 1 }}>
                {item.budgetName}
              </Typography>
